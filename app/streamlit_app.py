...
with col1:
    st.subheader(f"{selected_metric} — {', '.join(selected_companies) if selected_companies else 'All companies'}")
    if main_df.empty:
        st.info("No data for selected filters.")
    else:
        # Line chart per company
        fig = px.line(main_df, x="year", y="value", color="company", markers=True,
                      labels={"value": selected_metric, "year": "Year"})
...
with col2:
    ...
    if main_df.empty:
        ...
        st.dataframe(latest.set_index("company"))

st.markdown("---")
st.subheader("Data table")
st.dataframe(main_df.reset_index(drop=True))
